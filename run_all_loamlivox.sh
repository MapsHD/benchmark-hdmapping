...
# Script to run LOAM-Livox (ROS1) benchmarks for ALL datasets
set -e
...
echo "======================================"
echo "Running LOAM-Livox benchmarks"
...
while IFS= read -r SENSOR_DIR; do
    ...
    START_TIME=$(date +%s)
    if ./methods/benchmark-LOAM-Livox-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
