...
# Script to run LIO-EKF (ROS1) benchmarks for ALL datasets
set -e
...
echo "======================================"
echo "Running LIO-EKF benchmarks"
...
while IFS= read -r SENSOR_DIR; do
    ...
    START_TIME=$(date +%s)
    if ./methods/benchmark-LIO-EKF-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
