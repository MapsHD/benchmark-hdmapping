...
# Script to run FAST-LIO (ROS1) benchmarks for ALL datasets
# Automatically discovers all sensor directories and processes them
#
# Usage: ./run_all_fastlio.sh BASE_PATH
...
echo "======================================"
echo "Running FAST-LIO (ROS1) benchmarks for all datasets"
echo "======================================"
...
while IFS= read -r SENSOR_DIR; do
    ...
    if [ -f "./methods/benchmark-FAST-LIO-to-HDMapping/configs/${CONFIG}.env" ]; then
...
while IFS= read -r SENSOR_DIR; do
    ...
    CONFIG_FILE="./methods/benchmark-FAST-LIO-to-HDMapping/configs/${CONFIG}.env"
    ...
    echo "Running FAST-LIO benchmark for $EXPERIMENT_NAME with config: $CONFIG..."
    START_TIME=$(date +%s)

    if ./methods/benchmark-FAST-LIO-to-HDMapping/run_benchmark.sh "$CONFIG" "$INPUT_BAG_FILE"; then
        END_TIME=$(date +%s)
...
echo "======================================"
echo "All FAST-LIO benchmarks completed!"
