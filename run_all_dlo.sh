...
# Script to run DLO (ROS1) benchmarks for ALL datasets
set -e
...
while IFS= read -r SENSOR_DIR; do
    ...
    START_TIME=$(date +%s)
    if ./methods/benchmark-DLO-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
