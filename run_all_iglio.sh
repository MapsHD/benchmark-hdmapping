...
# Script to run iG-LIO (ROS1) benchmarks for ALL datasets
# Usage: ./run_all_iglio.sh BASE_PATH
...
echo "======================================"
echo "Running iG-LIO (ROS1) benchmarks for all datasets"
echo "======================================"
...
while IFS= read -r SENSOR_DIR; do
    ...
    if [ -f "./methods/benchmark-iG-LIO-to-HDMapping/configs/${CONFIG}.env" ]; then
...
while IFS= read -r SENSOR_DIR; do
    ...
    CONFIG_FILE="./methods/benchmark-iG-LIO-to-HDMapping/configs/${CONFIG}.env"
    ...
    echo "Running iG-LIO benchmark for $EXPERIMENT_NAME with config: $CONFIG..."
    START_TIME=$(date +%s)

    if ./methods/benchmark-iG-LIO-to-HDMapping/run_benchmark.sh "$CONFIG" "$INPUT_BAG_FILE"; then
        END_TIME=$(date +%s)
...
echo "======================================"
echo "All iG-LIO benchmarks completed!"
