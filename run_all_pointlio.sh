...
# Script to run Point-LIO (ROS1) benchmarks for ALL datasets
# Automatically discovers all sensor directories and processes them
#
# Usage: ./run_all_pointlio.sh BASE_PATH
...
echo "======================================"
echo "Running Point-LIO (ROS1) benchmarks for all datasets"
echo "======================================"
...
while IFS= read -r SENSOR_DIR; do
    ...
    if [ -f "./methods/benchmark-Point-LIO-to-HDMapping/configs/${CONFIG}.env" ]; then
...
while IFS= read -r SENSOR_DIR; do
    ...
    CONFIG_FILE="./methods/benchmark-Point-LIO-to-HDMapping/configs/${CONFIG}.env"
    ...
    echo "Running Point-LIO benchmark for $EXPERIMENT_NAME with config: $CONFIG..."
    START_TIME=$(date +%s)

    if ./methods/benchmark-Point-LIO-to-HDMapping/run_benchmark.sh "$CONFIG" "$INPUT_BAG_FILE"; then
        END_TIME=$(date +%s)
...
echo "======================================"
echo "All Point-LIO benchmarks completed!"
