...
# Script to run CT-ICP (ROS1) benchmarks for ALL datasets
set -e
...
echo "======================================"
echo "Running CT-ICP benchmarks"
...
while IFS= read -r SENSOR_DIR; do
    ...
    START_TIME=$(date +%s)
    if ./methods/benchmark-CT-ICP-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
