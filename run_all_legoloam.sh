...
# Script to run LeGO-LOAM (ROS1) benchmarks for ALL datasets
set -e
...
echo "======================================"
echo "Running LeGO-LOAM benchmarks"
...
while IFS= read -r SENSOR_DIR; do
    ...
    START_TIME=$(date +%s)
    if ./methods/benchmark-LeGO-LOAM-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
