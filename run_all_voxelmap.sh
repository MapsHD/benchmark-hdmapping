...
# Script to run VoxelMap (ROS1) benchmarks for ALL datasets
# Usage: ./run_all_voxelmap.sh BASE_PATH
...
echo "======================================"
echo "Running VoxelMap (ROS1) benchmarks for all datasets"
...
while IFS= read -r SENSOR_DIR; do
    ...
    if [ ! -f "$INPUT_BAG_FILE" ]; then
        echo "  Skipping (no bag)"
        RESULTS+=("SKIP $EXPERIMENT_NAME")
        continue
    fi

    START_TIME=$(date +%s)
    if ./methods/benchmark-VoxelMap-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
