...
# Script to batch convert all Mandeye data directories to ROS1 bags in parallel
# Usage: ./batch-convert-ros1-parallel.sh <root_directory> [options]
#
# This uses the Docker-based converter in:
#   tools/mandeye_to_bag/mandeye-to-bag-ros1.sh

set -e

if [ "$#" -lt 1 ]; then
    echo "Usage: $0 <root_directory> [--workers <number>] [--dry-run] [--lines <number>]"
    echo ""
    echo "Arguments:"
    ...
    echo "  $0 ~/data/benchmark-joseph --workers 16 --lines 8"
    exit 1
fi
...
CONVERTER_SCRIPT="$SCRIPT_DIR/tools/mandeye_to_bag/mandeye-to-bag-ros1.sh"

if [ ! -f "$CONVERTER_SCRIPT" ]; then
    echo "Error: ROS1 converter script not found: $CONVERTER_SCRIPT"
...
if [ "$DRY_RUN" = true ]; then
    echo "Parallel Mandeye to ROS1 Bag Conversion (DRY RUN)"
else
    echo "Parallel Mandeye to ROS1 Bag Conversion"
fi
echo "============================================"
...
if [ "$DRY_RUN" = true ]; then
    ...
    while IFS= read -r INPUT_DIR; do
        CURRENT=$((CURRENT + 1))
        OUTPUT_DIR="$INPUT_DIR/ros1bag"
        OUTPUT_BAG="$OUTPUT_DIR/mandeye.bag"
    ...
    if command -v parallel &> /dev/null; then
        ...
        echo "$DIRS" | parallel --jobs "$WORKERS" --progress --joblog /tmp/batch-convert-ros1-$$.log \
        ...
        if [ -f /tmp/batch-convert-ros1-$$.log ]; then
            SUCCESS=$(awk 'NR>1 && $7==0 {count++} END {print count+0}' /tmp/batch-convert-ros1-$$.log)
            FAILED=$(awk 'NR>1 && $7!=0 {count++} END {print count+0}' /tmp/batch-convert-ros1-$$.log)
            echo "Total directories: $DIR_COUNT"
            echo "Successful: $SUCCESS"
            echo "Failed: $FAILED"
            rm -f /tmp/batch-convert-ros1-$$.log
        fi
