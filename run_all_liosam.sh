...
# Script to run LIO-SAM (ROS1) benchmarks for ALL datasets
set -e
...
echo "======================================"
echo "Running LIO-SAM benchmarks"
...
while IFS= read -r SENSOR_DIR; do
    ...
    START_TIME=$(date +%s)
    if ./methods/benchmark-LIO-SAM-to-HDMapping/run_benchmark.sh avia "$INPUT_BAG_FILE"; then
